...
def LoadModuleLocal(mod_name, gl=globals(), skip_dun=True, only_funcs=False):
    ...
    try:
        ...
        print('error')
    exec('\n'.join([(name + '=' + mod_name + '.'+name if (((name[:2]!='__') or not skip_dun) and ((eval('callable(' + mod_name + '.' + name + ')', gl)) or not only_funcs)) else '' ) for name in eval(mod_name, gl).__dict__]).lstrip()+'\n', gl)